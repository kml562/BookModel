...
// Check if the bookId exists and is not deleted before adding the review. json an error response with appropirate status code like this if the book does not exist
...
export const postReview = async (req, res) => {
  try {
    ...
    let updateReview = await BookModel.findByIdAndUpdate(
      bookId,
      { $inc: { reviews: 1 } },
      { new: true }
    );
...
export const updateReview = async function (req, res) {
  try {
    ...
    if (!reviewsData) {
      return res
        .status(404)
        .json({ staus: false, message: "This reviewId does not found" });
    }
    ...
    if (Object.keys(data).length == 0)
      return res
        .status(400)
        .json({ staus: false, message: "put json some data to update" });
    ...
    if (data.rating) {
      if (typeof data.rating !== "number")
        return res
          .status(400)
          .json({ status: false, message: "rating should be Number" });
      if (!(data.rating >= 0 && data.rating < 6)) {
        return res
          .status(400)
          .json({ status: false, message: "rating should be from 1 to 5" });
      }
      object.rating = data.rating;
    }
    //checking reviewBy key and also checking that it should have alphabets only--------------------
    if (data.reviewedBy) {
      if (!validator.isAlpha(data.reviewedBy, "en-US", { ignore: " " }))
        return res.status(400).json({
          status: false,
          message:
            "It accepts only alphabets and there should not be any special characters numbers and spaces",
        });
    ...
    return res
      .status(200)
      .json({ status: true, message: "success", data: bookdata });
  } catch (error) {
...
export const deleteReview = async (req, res) => {
  try {
    ...
    if (!review) {
      return res
        .status(404)
        .json({ status: false, message: "review id not found" });
    }
